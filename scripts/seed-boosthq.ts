...
// BoostHQ - idea_custom_002
const boostHqIdea = {
  id: "boosthq-001",
  userId: null,
  title: "BoostHQ",
  oneLiner: "Grow your newsletter 3x faster with cross-promos, referrals, and insights that work everywhere",
  problem: "Newsletter creators are stuck in growth limbo. They write great content but struggle to get discovered. Cross-promotions work but finding partners manually takes hours. Referral programs drive subscribers but Beehiiv charges extra, Substack doesn't have them, and Ghost requires plugins. Analytics are siloed—you can't see the full picture across platforms. The result? Writers hit a growth ceiling around 5-10K subscribers and don't know how to break through.",
  targetCustomer: "Newsletter creators with 1K-50K subscribers who want to break through growth plateaus and monetize their audience. Secondary: Newsletter operators at media companies and multi-author publications managing growth across multiple properties.",
  solution: "The all-in-one growth toolkit for newsletter creators. Find and manage cross-promotion partners in your niche. Launch referral programs that work on any platform. See unified analytics across Substack, Beehiiv, Ghost, and ConvertKit. Get AI-powered growth recommendations based on what's working for similar newsletters. Stop guessing. Start growing.",
  monetization: "Freemium: Free (1 newsletter, 5 cross-promos/mo), Growth $29/month (3 newsletters, unlimited promos, referrals), Pro $79/month (unlimited newsletters, advanced analytics), Team $199/month (5 seats, multi-publication).",
  tags: ["newsletter-tools", "growth-marketing", "saas", "analytics", "email-marketing"],
  keywords: null,
  ...
  isPublic: true,
  isSaved: false,
  whyNow: "Newsletter renaissance is real: (1) Substack has 35M+ subscribers, Beehiiv growing 10x YoY—independent newsletters are a legitimate business, (2) Cross-promotion has emerged as the #1 growth hack but has no dedicated tooling, (3) Referral programs drove Morning Brew to 4M subscribers—everyone wants to replicate this, (4) Platform lock-in is a real concern—creators need platform-agnostic tools.",
  revenueImpact: "Break through growth plateaus. 3x faster subscriber acquisition through proven cross-promo and referral strategies. Unified analytics save 10+ hours/week of manual tracking across platforms.",
  marketSize: "$10B",
  competitionLevel: "Medium" as const,
  category: "marketing-tools" as const,
  businessType: "b2c" as const,
  priceRange: "smb" as const
};

const boostHqLanding = {
  meta: {
...
async function seedBoostHq() {
  console.log("Seeding BoostHQ...");

  try {
    // Check if it already exists
    const existing = await db.select().from(ideas).where(eq(ideas.id, boostHqIdea.id));
    
    if (existing.length > 0) {
      console.log("BoostHQ idea already exists. Updating...");
      await db.update(ideas)
        .set({ 
          ...boostHqIdea,
          updatedAt: new Date()
        })
        .where(eq(ideas.id, boostHqIdea.id));
    } else {
      // Insert the idea
      ...
      console.log("Created BoostHQ idea.");
    }

    // Check if landing page exists
    const existingLanding = await db.select().from(landingPages).where(eq(landingPages.slug, "boosthq"));
    
    if (existingLanding.length > 0) {
      console.log("BoostHQ landing page already exists. Updating...");
      await db.update(landingPages)
        .set({ 
          content: JSON.stringify(boostHqLanding),
          updatedAt: new Date(),
          isPublished: true 
        })
        .where(eq(landingPages.slug, "boosthq"));
    } else {
      // Insert the landing page
      await db.insert(landingPages).values({
        ideaId: boostHqIdea.id,
        slug: "boosthq",
        content: JSON.stringify(boostHqLanding),
        isPublished: true,
        createdAt: new Date(),
      ...
      console.log("Created BoostHQ landing page.");
    }
      ...
      launchChecklist: [
        ...
        JSON.stringify({
          launch_checklist: {
            pre_launch: {
              "4_weeks_before": [
                "MVP with Beehiiv + ConvertKit integrations (easiest APIs)",
                "Basic cross-promo directory and request flow",
                "Landing page live with waitlist",
                "Create demo showing cross-promo and analytics",
                "Set up PostHog for analytics",
                "Start Twitter presence on newsletter growth tips"
              ],
              "2_weeks_before": [
                "Add Substack integration (workarounds for limited API)",
                "Beta test with 30-50 newsletter creators",
                "Launch referral program MVP",
                "Collect beta testimonials and growth stories",
                "Prepare Product Hunt assets",
                "Set up Stripe billing"
              ],
              "1_week_before": [
                "Final product polish and bug fixes",
                "Seed directory with 100+ newsletter profiles",
                "Reach out to newsletter creator communities",
            ...
            launch_day: [
              ...
              {
                time: "8:00 AM",
                task: "LinkedIn post targeting media/newsletter operators"
              },
              {
                time: "9:00 AM",
              ...
              {
                time: "10:00 AM",
                task: "Post in newsletter creator communities"
              },
              {
                time: "11:00 AM",
                task: "Share in Substack Notes and relevant Substacks"
              }
            ],
            post_launch: {
              week_1: [
    ...
    const existingPlan = await db.select().from(executionPlans).where(eq(executionPlans.ideaId, boostHqIdea.id));
    
    if (existingPlan.length > 0) {
      console.log("BoostHQ execution plan already exists. Updating...");
      await db.update(executionPlans)
        ...
        .where(eq(executionPlans.ideaId, boostHqIdea.id));
    } else {
      // Insert new execution plan
      await db.insert(executionPlans).values(executionPlanData);
      console.log("Created BoostHQ execution plan with comprehensive deliverables.");
    }

    console.log("Done! BoostHQ has been added to the idea library with all comprehensive content.");
  } catch (error) {
    console.error("Error seeding BoostHQ:", error);
    process.exit(1);
  }
